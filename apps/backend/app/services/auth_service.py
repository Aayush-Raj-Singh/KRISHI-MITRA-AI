...
class AuthService:
    ...
    async def get_user_by_phone(self, phone: str) -> Optional[UserInDB]:
        ...
        if not user:
            return None
        return UserInDB.from_record(user)
    ...
    async def update_profile(self, user: UserInDB, payload: dict) -> Optional[UserInDB]:
        if not user.id:
            return None
        now = self._now()
        ...
        for key in (
            ...
            if key in payload and payload[key] is not None:
                ...
                if key in {"location", "farm_size", "soil_type", "water_source", "primary_crops"}:
                    updates[f"farm_profile.{key}"] = payload[key]

        if "profile_image_url" in payload and payload["profile_image_url"] is None:
            updates["profile_image_url"] = None

        if "risk_view_consent" in payload and payload["risk_view_consent"] is not None:
            updates["risk_view_consent"] = payload["risk_view_consent"]

        preferences = payload.get("preferences") or {}
        if "notifications" in preferences and preferences["notifications"] is not None:
            updates["preferences.notifications"] = preferences["notifications"]
        if "voice_input" in preferences and preferences["voice_input"] is not None:
            updates["preferences.voice_input"] = preferences["voice_input"]

        if len(updates) == 1:
            return user

        await self._collection.update_one(self._user_query(user.id), {"$set": updates})
        updated = await self._collection.find_one(self._user_query(user.id))
        if not updated:
            return None
        return UserInDB.from_record(updated)
    ...
    async def create_user(self, payload: dict) -> UserInDB:
        ...
        return UserInDB.from_record(created)
