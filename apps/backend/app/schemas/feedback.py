...
class OutcomeFeedbackRequest(StrictSchema):
...
class SustainabilityScores(StrictSchema):
    water_efficiency: float
    fertilizer_efficiency: float
    yield_optimization: float


class OutcomeFeedbackResponse(StrictSchema):
    feedback_id: str
...
class QuickFeedbackRequest(StrictSchema):
...
class QuickFeedbackResponse(StrictSchema):
    feedback_id: str
    recommendation_id: Optional[str] = None
    rating: int
    service: str
