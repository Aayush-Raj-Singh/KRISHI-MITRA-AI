...
async def crop_recommendations(
    ...
    try:
        if settings.sagemaker_runtime_enabled:
            ...
            response = await asyncio.to_thread(SageMakerInferenceService().crop_recommend, request_payload)
        else:
...
async def price_forecast(
    ...
    rec_id = await recommendation_service.store(user.id, "price", price_request, response.model_dump())
    response.recommendation_id = rec_id
