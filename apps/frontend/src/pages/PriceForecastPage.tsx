...
interface PriceForecastContentProps {
  embedded?: boolean;
}

export const PriceForecastContent: React.FC<PriceForecastContentProps> = ({ embedded = false }) => {
  const { t } = useTranslation();
  const isOnline = useOnlineStatus();
  const mandiOptions = useMandiFilterOptions();
  const cropSuggestions = useLocalSuggestions("price_forecast.crop", mandiOptions.crops);
  ...
  const handleLoadCached = () => {
    try {
      ...
      if (!raw) return;
      const payload = JSON.parse(raw) as Partial<typeof form> & { market?: string };
      const nextForm = {
        state: payload.state || "",
  ...
  const handleGenerate = () => {
    cropSuggestions.addSuggestion(form.crop);
    currencySuggestions.addSuggestion(form.currency);
    mutation.mutate({ crop: form.crop, market: form.mandi, currency: form.currency });
  };
