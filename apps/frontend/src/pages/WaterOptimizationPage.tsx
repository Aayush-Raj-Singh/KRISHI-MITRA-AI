...
interface WaterOptimizationContentProps {
  embedded?: boolean;
}

export const WaterOptimizationContent: React.FC<WaterOptimizationContentProps> = ({
  embedded = false,
}) => {
    ...
    onSuccess: (data) => {
      ...
      try {
        localStorage.setItem(
          "krishimitra:last_water_payload",
          JSON.stringify({ ...form, forecast }),
        );
      } catch {}
    },
  });
  ...
  const cacheKey = useMemo(
    () => `water:${JSON.stringify({ ...form, forecast })}`,
    [form, forecast],
  );
  ...
  const handleLoadCached = () => {
    try {
      ...
      const cached = getCachedWithMeta<WaterOptimizationResponse>(
        `water:${JSON.stringify(payload)}`,
      );
      if (cached) {
    ...
    <Stack spacing={3}>
      {embedded ? (
        ...
        <AgricultureHero
          ...
          badges={[
            t("dashboard_page.water.growth_stage"),
            t("dashboard_page.water.soil_moisture_pct"),
            t("dashboard_page.kpi.water_savings"),
          ]}
              ...
              <Stack spacing={2}>
                ...
                <Stack direction="row" spacing={2}>
                  <TextField
                    ...
                    onChange={(e) =>
                      setForm((prev) => ({ ...prev, soil_moisture_pct: Number(e.target.value) }))
                    }
                    fullWidth
                  />
                  <TextField
                    ...
                    onChange={(e) =>
                      setForm((prev) => ({ ...prev, field_area_acres: Number(e.target.value) }))
                    }
                    fullWidth
                  />
                </Stack>
