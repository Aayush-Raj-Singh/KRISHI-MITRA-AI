...
export const RecommendationsScreen = () => {
  ...
  const renderCrop = () => (
    <SectionCard title={toolLabels.crop.title} subtitle={toolLabels.crop.subtitle}>
      <FieldInput label="Soil nitrogen" keyboardType="decimal-pad" onChangeText={(value) => setCropForm((prev) => ({ ...prev, soil_n: value }))} value={cropForm.soil_n} />
      <FieldInput label="Soil phosphorus" keyboardType="decimal-pad" onChangeText={(value) => setCropForm((prev) => ({ ...prev, soil_p: value }))} value={cropForm.soil_p} />
      <FieldInput label="Soil potassium" keyboardType="decimal-pad" onChangeText={(value) => setCropForm((prev) => ({ ...prev, soil_k: value }))} value={cropForm.soil_k} />
      <FieldInput label="Soil pH" keyboardType="decimal-pad" onChangeText={(value) => setCropForm((prev) => ({ ...prev, soil_ph: value }))} value={cropForm.soil_ph} />
      <FieldInput label="Temperature (C)" keyboardType="decimal-pad" onChangeText={(value) => setCropForm((prev) => ({ ...prev, temperature_c: value }))} value={cropForm.temperature_c} />
      <FieldInput label="Humidity (%)" keyboardType="decimal-pad" onChangeText={(value) => setCropForm((prev) => ({ ...prev, humidity_pct: value }))} value={cropForm.humidity_pct} />
      <FieldInput label="Rainfall (mm)" keyboardType="decimal-pad" onChangeText={(value) => setCropForm((prev) => ({ ...prev, rainfall_mm: value }))} value={cropForm.rainfall_mm} />
      <FieldInput label="Location" onChangeText={(value) => setCropForm((prev) => ({ ...prev, location: value }))} value={cropForm.location} />
      <FieldInput label="Season" onChangeText={(value) => setCropForm((prev) => ({ ...prev, season: value }))} value={cropForm.season} />
      <FieldInput label="Historical yield" keyboardType="decimal-pad" onChangeText={(value) => setCropForm((prev) => ({ ...prev, historical_yield: value }))} value={cropForm.historical_yield} />
      <PrimaryButton label="Generate crop plan" loading={loadingTool === "crop"} onPress={() => void submitCrop()} />
  ...
  const renderPrice = () => {
    ...
    return (
      <SectionCard title={toolLabels.price.title} subtitle={toolLabels.price.subtitle}>
        <FieldInput label="Crop" onChangeText={(value) => setPriceForm((prev) => ({ ...prev, crop: value }))} value={priceForm.crop} />
        <FieldInput label="Market" onChangeText={(value) => setPriceForm((prev) => ({ ...prev, market: value }))} value={priceForm.market} />
        <FieldInput autoCapitalize="characters" label="Currency" onChangeText={(value) => setPriceForm((prev) => ({ ...prev, currency: value }))} value={priceForm.currency} />
        <PrimaryButton label="Forecast prices" loading={loadingTool === "price"} onPress={() => void submitPrice()} />
  ...
  const renderWater = () => (
    <SectionCard title={toolLabels.water.title} subtitle={toolLabels.water.subtitle}>
      <FieldInput label="Crop" onChangeText={(value) => setWaterForm((prev) => ({ ...prev, crop: value }))} value={waterForm.crop} />
      <FieldInput label="Growth stage" onChangeText={(value) => setWaterForm((prev) => ({ ...prev, growth_stage: value }))} value={waterForm.growth_stage} />
      <FieldInput label="Soil moisture (%)" keyboardType="decimal-pad" onChangeText={(value) => setWaterForm((prev) => ({ ...prev, soil_moisture_pct: value }))} value={waterForm.soil_moisture_pct} />
      <FieldInput label="Water source" onChangeText={(value) => setWaterForm((prev) => ({ ...prev, water_source: value }))} value={waterForm.water_source} />
      <FieldInput label="Field area (acres)" keyboardType="decimal-pad" onChangeText={(value) => setWaterForm((prev) => ({ ...prev, field_area_acres: value }))} value={waterForm.field_area_acres} />
      <FieldInput label="Location" onChangeText={(value) => setWaterForm((prev) => ({ ...prev, location: value }))} value={waterForm.location} />
      <PrimaryButton label="Optimize irrigation" loading={loadingTool === "water"} onPress={() => void submitWater()} />

      {waterResult ? (
        <View style={styles.resultBlock}>
          <Text style={styles.resultTitle}>Estimated savings: {waterResult.water_savings_percent.toFixed(1)}%</Text>
    ...
    >
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.toolRow}>
        {(Object.keys(toolLabels) as ToolId[]).map((tool) => (
          ...
          >
            <Text style={[styles.toolChipText, activeTool === tool ? styles.toolChipTextActive : null]}>
