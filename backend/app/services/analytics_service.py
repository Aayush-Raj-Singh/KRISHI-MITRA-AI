...
class AnalyticsService:
    ...
    def _allowed_regions(self, actor: UserInDB | None) -> List[str]:
        if not actor or actor.role == "admin":
            return []
        assigned = [region.strip() for region in getattr(actor, "assigned_regions", []) if region.strip()]
        if assigned:
            return assigned
        if actor.location:
    ...
    def _effective_regions(self, requested_location: Optional[str], actor: UserInDB | None) -> List[str]:
        ...
        for region in allowed:
            if requested_norm in self._normalized(region) or self._normalized(region) in requested_norm:
                return [region]
        return []
    ...
    async def _filtered_user_ids(
        ...
        if effective_regions:
            query["$or"] = [{"location": {"$regex": region, "$options": "i"}} for region in effective_regions]
        elif self._allowed_regions(actor):
            # Actor has location restrictions but requested location is outside allowed scope.
            return []
    ...
    def _date_range(self, from_date: Optional[datetime], to_date: Optional[datetime]) -> Dict[str, datetime]:
    ...
    def _parse_date_filters(self, from_date: Optional[str], to_date: Optional[str]) -> Tuple[Optional[datetime], Optional[datetime]]:
        parsed_from = None
        parsed_to = None
        if from_date:
            parsed_from = datetime.combine(datetime.fromisoformat(from_date).date(), time.min, tzinfo=timezone.utc)
        if to_date:
            parsed_to = datetime.combine(datetime.fromisoformat(to_date).date(), time.max, tzinfo=timezone.utc)
    ...
    async def overview(
        ...
        user_ids = await self._filtered_user_ids(location, crop, farm_size_min, farm_size_max, actor=actor)
        ...
        feedback_query: Dict[str, object] = {"user_id": {"$in": user_ids}} if user_ids else {"user_id": "__none__"}
        if date_query:
        ...
        yield_values = [doc.get("outcomes", {}).get("yield_kg_per_acre", 0.0) for doc in feedback_docs]
        water_values = [doc.get("outcomes", {}).get("water_usage_l_per_acre", 0.0) for doc in feedback_docs]
        fertilizer_values = [doc.get("outcomes", {}).get("fertilizer_kg_per_acre", 0.0) for doc in feedback_docs]
        ...
        top_crops = [
            ...
            for crop_name, count in sorted(crop_counts.items(), key=lambda item: item[1], reverse=True)[:6]
        ]
        ...
        logger.info("analytics_overview_generated", total_farmers=total_farmers, total_feedback=total_feedback)
        return overview
    ...
    async def farmers_needing_attention(
        ...
        effective_regions = self._effective_regions(location, actor)
        if effective_regions:
            user_query["$or"] = [{"location": {"$regex": region, "$options": "i"}} for region in effective_regions]
        elif self._allowed_regions(actor):
            return []

        users = await self._users.find(user_query, {"name": 1, "location": 1, "risk_view_consent": 1}).to_list(length=None)
        if not users:
            return []
        ...
        for user in users:
            ...
            feedback_docs = (
                await self._feedback.find({"user_id": user_id}).sort("created_at", 1).limit(12).to_list(length=12)
            )
            if not feedback_docs:
                continue

            sustainability_values = [float(item.get("sustainability_score", 0.0)) for item in feedback_docs]
            latest_sustainability = sustainability_values[-1]
            ratings = [float(item.get("rating", 0.0)) for item in feedback_docs]
            avg_rating = round(sum(ratings) / len(ratings), 2) if ratings else 0.0

            yields = [float(item.get("outcomes", {}).get("yield_kg_per_acre", 0.0)) for item in feedback_docs]
    ...
    async def feedback_reliability(
        ...
        total_feedback = len(feedback_docs)
        ratings = [int(item.get("rating", 0)) for item in feedback_docs if int(item.get("rating", 0)) > 0]
        average_rating = round(sum(ratings) / len(ratings), 2) if ratings else 0.0
        negative_outcomes = len([item for item in feedback_docs if bool(item.get("negative_outcome", False))])
        negative_outcome_rate = round((negative_outcomes / total_feedback) * 100, 2) if total_feedback else 0.0
