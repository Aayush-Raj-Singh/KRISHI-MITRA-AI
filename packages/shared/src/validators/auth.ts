...
export const ensureStrongPassword = (value: string): string => {
  ...
  if (password.length < 8 || rules.some((rule) => !rule.test(password))) {
    throw new Error(
      "Password must be 8+ characters and include upper, lower, number, and special characters",
    );
  }
  return password;
};
...
export const sanitizePasswordResetRequest = (
...
export const sanitizeUserProfileUpdate = (payload: UserProfileUpdate): UserProfileUpdate => ({
  ...
  water_source: payload.water_source
    ? requireText(payload.water_source, "Water source")
    : undefined,
