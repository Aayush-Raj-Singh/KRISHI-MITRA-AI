...
export const createAuthApi = ({ api, unwrap }: FeatureApiContext) => ({
  ...
  confirmPasswordReset: async (payload: PasswordResetConfirmRequest): Promise<{ reset: boolean }> => {
  ...
  refresh: async (payload: { refresh_token: string }): Promise<TokenResponse> => {
    const response = await api.post<ApiEnvelope<TokenResponse>>(API_ENDPOINTS.auth.refresh, payload);
    return unwrap(response.data);
  },
  logout: async (payload: LogoutRequest): Promise<{ revoked: boolean }> => {
    const response = await api.post<ApiEnvelope<{ revoked: boolean }>>(API_ENDPOINTS.auth.logout, payload);
    return unwrap(response.data);
  },
